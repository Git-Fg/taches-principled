...
| `<constraints>` | Hard rules (NEVER/MUST/SHOULD) |
...
The `description` field is critical for automatic invocation. LLMs use descriptions to make routing decisions.

❌ Bad (too vague): `description: Helps with testing`
❌ Bad (not differentiated): `description: Billing agent`
✅ Good (specific triggers): `description: Creates comprehensive test suites. Use when new code needs tests or test coverage is insufficient.`
...
**Why this fails:** Subagents execute in isolated contexts ("black boxes"). They cannot use AskUserQuestion or any tool requiring user interaction.
