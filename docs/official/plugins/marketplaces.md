...
## taches-principled: two-source catalog model

The taches-principled marketplace uses a two-source catalog model
where `marketplace.json` is **derived** from two sources, not
hand-edited:

1. **Per-plugin manifest** (`plugins/*/.claude-plugin/plugin.json`) —
   holds `name`, `version`, `description` for each plugin. This is
   the spec-authoritative source per the marketplace's CHANGELOG 1.12.0.
2. **Catalog metadata** (`.claude-plugin/_meta.json`) — holds
   `source`, `homepage`, `repository`, `license`, `category`,
   `keywords` per plugin.

A maintainer updating a plugin's description edits only the
per-plugin `plugin.json`. A maintainer updating keywords or category
edits only `_meta.json`. The marketplace.json is regenerated by the
maintainer's workflow. The marketplace's CLAUDE.md "Version & Release"
section documents the regeneration one-liner.

CI validation: `.github/workflows/validate-marketplace.yml` checks
that (1) no `version:` key is duplicated per plugin entry, (2)
per-plugin `plugin.json` version matches `marketplace.json`, (3) no
plugin is orphaned or unlisted, (4) every `_meta.json` plugin is
also in `marketplace.json`.
