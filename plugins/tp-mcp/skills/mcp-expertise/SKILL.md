---
name: mcp-expertise
description: "Design, build, and evaluate MCP servers end-to-end. Use when the user says 'design an MCP server', 'decompose MCP tools', 'write a tool schema', 'implement an MCP server in Rust', 'call an MCP server from an agent', or 'evaluate MCP quality'. Five modes: DESIGN (decomposition, contracts), SCHEMA (JSON Schema), IMPLEMENT (Rust + rmcp), CLIENT (agent as consumer), QUALITY (8-dimension rubric, parallel quality judges in isolated contexts). NOT for: general REST API or service design (use `ddd`); NOT for: general code quality review (use `refine`)."
when_to_use: |
...
- "Spawn in-process subagents within a session" → use `subagent-orchestration`
- "Build a non-MCP CLI tool" → use `subagent-orchestration` or shell tooling
- "Investigate a runtime bug or root cause" → use `diagnose`
...
**CONTRAST with `claude-cli`:** `claude-cli` drives the `claude` binary as a tool from Bash; `mcp-expertise` builds and uses MCP servers. The boundary is the same: anything that touches MCP server design/schema/implementation/usage lives here, anything that drives the `claude` CLI itself lives in `claude-cli`.
