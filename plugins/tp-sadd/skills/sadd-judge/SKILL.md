---
name: sadd-judge
description: "Evaluate work using meta-judge then judge subagents: single judge for routine checks, 3-judge debate for high-stakes consensus, and multi-round debate for maximum rigor"
when_to_use: |
  When user says 'judge this', 'evaluate', 'assess quality', 'verify this', 'check my work', 'review this solution', 'grade this', 'is this any good', 'debate this', 'multi-judge debate', 'reach consensus', 'adversarial evaluation'. IMMEDIATELY when user asks for LLM-as-Judge verification or quality assessment. FIRST when evaluation requires independent criteria (not hardcoded). Do NOT use for routine quality checks when single-judge mode would suffice — but the skill handles all three modes so choose whichever matches the user's intent.
...
IF maximum rigor requires iterative debate refinement with explicit position changes → multi-round debate mode
...
## Mode 3: Multi-Round Debate (Maximum Rigor)

For high-stakes evaluation where iterative argumentation and explicit position refinement drive consensus. This mode emphasizes evidence-based debate where judges defend positions with citations and challenge counter-positions with counter-evidence.

### Process

**Phase 0.5: Meta-Judge** — Dispatch one meta-judge with task description, solution context, and artifact type. Generate evaluation specification YAML covering all relevant quality dimensions. This runs ONCE and is shared by all judges across all rounds.
...
**Phase 1: Independent Analysis** — Spawn 3 judges in parallel (Opus recommended). Each receives path to solution(s), exact meta-judge evaluation specification YAML, and task description. Each produces independent assessment saved to `.specs/reports/{solution-name}-{date}.[1|2|3].md` with per-criterion scores, evidence, overall score, and key strengths/weaknesses.

**Key principle:** Independence in initial analysis prevents groupthink.

**Phase 2: Debate Rounds (Max 3)** — For each round, launch 3 judges in parallel. Each reads:
...
**Phase 3: Consensus Check** — After each debate round, verify:
- All overall scores within 0.5 points of each other
...
### Key Principles for Multi-Round Debate

- Meta-judge runs once (shared spec grounds all debate)
- Judges read other reports from filesystem directly (no orchestrator paraphrasing)
...
| `failed` | `meta-judge-invalid-spec` | Malformed or unparseable evaluation YAML | `true` (relaunch meta-judge) |
| `failed` | `judge-failure` | Judge crashed or produced no output | `true` (relaunch judge) |
| `failed` | `consensus-not-reached` | 3 debate rounds completed without agreement | `false` (escalate to human) |
| `failed` | `debate-round-failure` | Round crashed mid-evaluation | `true` (resume from last valid round) |
| `failed` | `no-work-to-evaluate` | Empty or missing artifact | `false` (user must provide work) |

**Fields:**
- `status`: `"failed"` when evaluation cannot complete; `"success"` when evaluation report produced
- `reason`: Specific failure mode from the options above
- `completed_portion`: What was completed (e.g., "Round 2/3 complete, 2/3 judges submitted")
- `retry_possible`: `true` for recoverable failures (timeout, crash); `false` for consensus exhaustion or missing input
