...
when_to_use: |
  Use when the user says "execute this", "implement with verification", "meta-judge this",
  "run the task", "build this with quality gates", or "implement [something]".
  IMMEDIATELY when user asks to implement anything that needs independent verification.
  FIRST when task requires parallel implementation, sequential steps, or competitive generation.
