...
When the skill uses qualitative judging (execute-plans critic), the labels are `HIGH` / `MEDIUM` / `LOW` findings instead of numeric scores — but chain-of-thought and the integrity rules below still apply.
