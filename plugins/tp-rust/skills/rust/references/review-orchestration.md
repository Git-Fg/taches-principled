...
REVIEW audits a Rust project across five independent dimensions. Spawn one `tp-critic` instance per lens, all five in parallel (same `Agent` call, no barrier). Each lens runs in its own isolated context to prevent diagnostic crosstalk.

| # | Lens prompt (passed verbatim to `tp-critic`) | Dimension | Primary reference(s) |
|---|---|---|---|
...
When spawning each `tp-critic` instance, pass:
