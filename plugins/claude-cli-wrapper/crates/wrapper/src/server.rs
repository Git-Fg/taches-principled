...
impl WrapperServer {
    ...
    #[tool(
        name = "claude_execute",
        description = "Run a prompt through `claude -p`. Returns a JSON envelope with exit_code, stdout, stderr, and session_id. Use this tool when the user wants Claude to do something: write code, run a command, analyze a file, etc. Pass `session_id` to continue a previous run; pass `continue_session: true` to continue the most recent session in the current directory."
    )]
    async fn claude_execute(
        &self,
        params: Parameters<ExecuteInput>,
    ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        let result = tools::execute(&self.claude_path, params.0)
            .await
    ...
    #[tool(
        name = "claude_session",
        description = "Session lifecycle: resume/continue/fork/list/info/close a session by `session_id`. The `session_id` returned by `claude_execute` is the input. `list` and `info` take no session_id; `resume`, `info`, `close` require it."
    )]
    async fn claude_session(
        &self,
        params: Parameters<SessionInput>,
    ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        let result = tools::session(&self.claude_path, params.0)
            .await
            .map_err(internal_error)?;
        Ok(into_call_tool_result(result))
    }
    ...
    #[tool(
        name = "claude_context",
        description = "Workspace context: add_directory (--add-dir), setup_worktree (--worktree), doctor. Use to grant Claude access to a directory it does not already see, or to spin up an isolated worktree."
    )]
    async fn claude_context(
        &self,
        params: Parameters<ContextInput>,
    ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        let result = tools::context(&self.claude_path, params.0)
            .await
            .map_err(internal_error)?;
        Ok(into_call_tool_result(result))
    }
    ...
    #[tool(
        name = "claude_review",
        description = "Code review pass against a target (current branch, branch name, or PR URL). Returns findings with file:line references. Read-only — does not modify files. Use when the user asks for a review."
    )]
    async fn claude_review(
        &self,
        params: Parameters<ReviewInput>,
    ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        let result = tools::review(&self.claude_path, params.0)
            .await
            .map_err(internal_error)?;
        Ok(into_call_tool_result(result))
    }
    ...
    #[tool(
        name = "claude_agent",
        description = "Background agent management. `spawn` requires name+description+prompt; `status` and `terminate` require name; `list` requires nothing. Spawned agents run with bypass-permissions and bare mode (no auto-memory, no hooks)."
    )]
    async fn claude_agent(
        &self,
        params: Parameters<AgentInput>,
    ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        let result = tools::agent(&self.claude_path, params.0)
            .await
            .map_err(internal_error)?;
        Ok(into_call_tool_result(result))
    }
    ...
    #[tool(
        name = "claude_config",
        description = "Runtime config: set_model / set_effort / set_permissions / load_settings. Persists to the user-level settings file. Use to change the model or effort level for the next call, or to apply a settings.json from disk."
    )]
    async fn claude_config(
        &self,
        params: Parameters<ConfigInput>,
    ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        let result = tools::config(&self.claude_path, params.0)
            .await
            .map_err(internal_error)?;
        Ok(into_call_tool_result(result))
    }
...
fn internal_error(e: anyhow::Error) -> rmcp::ErrorData {
