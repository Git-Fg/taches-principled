# Argument Contract and Registry Preamble — `wiki-searcher` and inline wiki ops

This file is the single source of truth for two things the wiki skill needs:

1. **The `wiki-searcher` argument contract** — what arguments the hub is expected to pass when spawning it, and what to do when the hub forgets.
...
The hub skill (`SKILL.md`) cites this file once. The `wiki-searcher` agent inherits the contract via the `wiki` skill frontmatter.

---

## Argument Contract for `wiki-searcher`

The hub spawns `wiki-searcher`. It is **expected to pass `wiki_path` (preferred) or `alias` (fallback) as an argument**, plus the role-specific `query`:
...
| `alias` | string (fallback) | The label from `~/.claude/wiki-root.md` (e.g., "pharma", "work", "personal"). If received instead of `wiki_path`, `wiki-searcher` resolves it from the registry. |
...
If neither `wiki_path` nor `alias` is provided, `wiki-searcher` should still try to infer the target by reading `~/.claude/wiki-root.md` directly:
...
For LINT, the orchestrator may also spawn `tp-critic` with lens "wiki audit — orphan pages, broken wikilinks, missing frontmatter, index drift, tag sprawl, intent violations, stale content" for an isolated-context consistency audit when the wiki is large enough that an inline walk would flood the orchestrator's context.