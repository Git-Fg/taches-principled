...
Keep files under 200 lines of code. Large files accumulate multiple responsibilities, making them harder to test, review, and reuse. Extract cohesive blocks of logic into named functions that each serve a single purpose. If extracted functions are only used within the same context, keep them in the same file. When a file exceeds 200 lines even after decomposition, split related functions into separate modules grouped by responsibility.
...
async function processUserRegistration(input: unknown) {
  ...
  // ... more normalization and hashing ...
  ...
  const user = await db.users.create({
    data: { email: normalizedEmail, /* ... more fields ... */ },
  })

  // Send notifications (lines 56-80+)
  await emailService.send({ to: normalizedEmail, subject: 'Welcome!' })
  await analyticsService.track('user_registered', { userId: user.id })
  await auditLog.record('registration', { userId: user.id })
