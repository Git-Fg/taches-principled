...
A "mechanism" is a pure function that computes a result and returns it. A "policy" is what the caller decides to do with that result — throw, log, branch, or ignore. See [policy-mechanism.md](./references/policy-mechanism.md) for the defining distinction.
...
- **Mechanism** = `applyNewFeature(baseData)` returns new data. **Policy** = the caller decides whether to call it.
- **Mechanism** = `formatResult(result)` returns a string. **Policy** = the caller decides to log it.

## Incorrect

`validateResult` hides a throw inside what reads like a passive validation check. The call site shows no branching, no `if`, no `throw` — the reader assumes execution continues normally after the call.
...
The mechanism (`isValid`) is a pure function that returns a value. The policy (what to do when invalid) is explicit at the call site.
