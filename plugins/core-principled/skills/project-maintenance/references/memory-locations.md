...
| `.principled/memory/learnings.md` | Project memory (plugin-managed) | Long-lived; reviewed periodically | Project-wide durable learnings |
| `.principled/scratch/` | Working scratchpad | Short-lived; cleaned after each phase | Current phase working state |
...
- **Project memory** (`.principled/memory/`) is the canonical sink for both `project-maintenance` PLAN-ARCHIVE and `refine` MEMORIZE mode. Read this when running MEMORY-AUDIT.
- **Scratch** (`.principled/scratch/`) is ephemeral. Treat all entries as candidates for archive after the current phase closes.

## Issue Types

- **Duplicates** — same content hash or >80% keyword overlap across two or more files. Use `scripts/dedup.py` to detect.
...
- `MEMORY-DEDUP` mode in the parent skill — uses `scripts/dedup.py` to find duplicate groups
- `MEMORY-ARCHIVE` mode — moves stale entries to `~/.claude/archive/memory/{category}/{date}/` with a recovery manifest
- `refine` MEMORIZE mode — the other writer to `.principled/memory/learnings.md`
