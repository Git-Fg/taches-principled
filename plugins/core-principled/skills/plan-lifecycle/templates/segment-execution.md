...
2. **For each segment**:
   ...
   - **Write implementation results** to `.principled/scratch/{plan-id}-execution.md`: files modified, verification results, any deviations detected.
   - **Self-verify** checkpoint conditions:
     ...
     - **FAIL** → spawn a `tp-critic` subagent (lens: "diagnose why this segment failed verification; identify the root cause and the minimal fix") for an isolated-context diagnosis, then fix and re-verify (max 2 fix attempts).
