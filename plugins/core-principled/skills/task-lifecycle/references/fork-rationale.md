...
`task-lifecycle` uses `context: fork` to run the 4-stage task lifecycle (CAPTURE → REFINE → IMPLEMENT → DOCUMENT) in an isolated subagent. This file justifies the fork flag per the `tp-discipline` R3 rule.
...
- **CAPTURE:** a draft spec at `.principled/specs/<task-id>.draft.md`
- **REFINE:** a refined spec at `.principled/specs/<task-id>.spec.md`
...
Pre-1.23.0, each stage spawned its own specialist subagent. Post-1.23.0, the orchestrator runs each stage inline within the fork and spawns only `tp-critic` for review at stage boundaries. The fork's value is the isolation of the 4-stage pipeline's intermediate output from the user's session, not inner-stage parallelism.